...
# 2024.12.10
# not use video gan loss
# linearly increase GAN loss weight in training AE after adopting GAN loss
...
from .utils import shift_dim, adopt_weight, comp_getattr, adopt_weight_linear
...
class VQGANDinoV2Action(pl.LightningModule):
    ...
    def forward(self, x, x_action, x_action_masked=None, opt_stage=None, log_image=False):
        ...
        if opt_stage is not None:
            if opt_stage == 0: # autoencoder
                ...
                disc_factor = adopt_weight_linear(self.global_step, threshold=self.args.discriminator_iter_start, increase_step=2000)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_ae = recon_loss + recon_loss_action + commitment_loss + commitment_loss_action + \
                 aeloss + perceptual_loss + gan_feat_loss + kl_reg_action
        opt_ae.zero_grad()
