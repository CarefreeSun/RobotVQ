...
for i in range(args.start_shard, args.start_shard+args.num_shards):
    ...
    dst_filepath_train = os.path.join(args.dst, 'train', f'{i}.jsonl')
    dst_filepath_test = os.path.join(args.dst, 'test', f'{i}.jsonl')
    os.makedirs(os.path.dirname(dst_filepath_train), exist_ok=True)
    os.makedirs(os.path.dirname(dst_filepath_test), exist_ok=True)
    dst_file_train = open(dst_filepath_train, 'w')
    dst_file_test = open(dst_filepath_test, 'w')
