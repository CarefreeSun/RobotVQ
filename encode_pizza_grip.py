# 2024.10.30
# Modified from encode_pizza_width_crop
# for encoding data focusing on actions where grippers move
...
args.dst_dir = '/mnt/data-rundong/robot_datasets/' + 'grip_' + args.weight_path.split('/')[-3] + '_step20000' + '_tokenized'
...
with torch.no_grad():
    for (dataset_name, image_root) in zip(args.dataset_names, args.image_root):
        ...
        mean[-1] = 0.
        std[-1] = 1.
        src_filepath = os.path.join(args.src, dataset_name, 'pizza_dataset_width_grip.jsonl')
                    ...
                    try:
                        ...
                        for stack_cnt in range(args.n_stacked_clips):
                            ...
                            if start+stack_cnt == 0: # video will be self.length duplicates of frame 0, and each action entry will be [0] * 7
                                continue # for this dataset, we do not need start clip
